...
@section('content')
	...
	<a href="{{ route('admin.compras') }}" class="btn btn-outline-warning btn-block mb-5">
		Regresar
	</a>
		...
		@foreach( $compra->productos as $producto )
			<div class="card bg-light border-warning">
				...
				<ul class="list-group list-group-flush">
					...
					<li class="list-group-item">Existencia: <span class="float-right">{{ $producto->existencia }}</span></li>
				</ul>
				</div>
				<div class="card-footer">
					<small>Fecha de registro: </small>
					<small class="text-muted float-right">{{ $producto->created_at }}</small>
				</div>
			</div>
		@endforeach
